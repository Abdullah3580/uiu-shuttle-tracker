...
import { getDatabase, ref, set, update, onValue, remove, onDisconnect, runTransaction } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-database.js";
...
// Key format: "From__To" (groupId এর মতো exactly same)
const routePaths = {
    "UIU__Natun Bazar": [
        [23.797323, 90.450258],[23.797112, 90.449378],[23.798388, 90.449067],
        [23.800219, 90.448643],[23.799586, 90.443466],[23.799227, 90.440736],
        [23.798957, 90.438537],[23.798810, 90.436954],[23.798648, 90.434996],
        [23.798575, 90.433323],[23.798565, 90.432550],[23.798521, 90.431874],
        [23.798403, 90.430957],[23.798319, 90.430211],[23.798255, 90.429128],
        [23.798098, 90.427395],[23.798049, 90.426101],[23.798005, 90.425727],
        [23.797878, 90.424772]
    ],
    "Natun Bazar__UIU": [
        [23.797976, 90.424723],[23.798133, 90.425635],[23.798201, 90.427063],
        ...
        [23.797917, 90.449255],[23.797117, 90.449464],[23.797318, 90.450253]
    ]
    // নতুন route যোগ করতে: "Kuril__UIU": [[lat,lng], ...],
};

// --- Route Snapping: GPS কে route এর সবচেয়ে কাছের point এ snap করো ---
...
let currentDriverMsg = null;    // ✅ Feature 2: driver live message
let wakeLock = null;            // ✅ Feature 4: Wake Lock

const busMarkers = {};
const routePolylines = {};      // fixed blue route lines
const busPolylines = {};        // ✅ Feature 3: orange path history polylines
const busPathHistory = {};      // ✅ Feature 3: coordinate history per groupId
...
function updateUI(isSharing) {
    ...
    // ✅ Feature 2: message panel শুধু sharing এর সময় দেখাবে
...
// ✅ Feature 2: Driver live message set/clear
...
// 🔒 Spam-protected fake bus report
    ...
    sessionIds.forEach(id => {
        ...
        import("https://www.gstatic.com/firebasejs/11.0.1/firebase-database.js").then(({ get, set: fbSet }) => {
            get(reportedByRef).then(snap => {
                if (snap.exists()) { showNotification("You already reported this bus.", "error"); return; }
...
function startSharing() {
    ...
    watchId = navigator.geolocation.watchPosition((position) => {
        ...
        // ✅ Route Snapping: GPS → route এর নিকটতম point
        const routeKey = `${fromLocation}__${toLocation}`;
        const snapped = snapToRoute(latitude, longitude, routeKey);

        set(myBusRef, { 
            lat: snapped.lat, lng: snapped.lng, timestamp: Date.now(),
            from: fromLocation, to: toLocation,
            speed: currentSpeed, reports: 0,
            msg: currentDriverMsg || null  // ✅ Feature 2
        });
    }, (error) => { 
        if(error.code === 1) { 
    ...
    // ✅ Feature 4: Wake Lock — screen off হলেও GPS চলবে
    if ('wakeLock' in navigator) {
        navigator.wakeLock.request('screen')
            .then(lock => { wakeLock = lock; })
            .catch(() => {}); // unsupported হলে silently fail
    }
...
function listenForAllBuses() {
    ...
    onValue(busesRef, (snapshot) => {
        ...
        Object.keys(busesData).forEach(sessionId => {
            ...
            if (routeGroupMap[groupId]) {
                ...
                if (bus.timestamp > group.latestTimestamp) {
                    ...
                    group.latestMsg = bus.msg || null; // ✅ Feature 2
                }
            } else {
                routeGroupMap[groupId] = {
                    ...
                    latestMsg: bus.msg || null, // ✅ Feature 2
                    users: [bus]
                };
        ...
        busGroups.forEach(group => {
            ...
            // ✅ Feature 2: driver message popup এ দেখাও
        ...
        Object.keys(busMarkers).forEach(groupId => {
            if (!activeGroupIds[groupId]) {
                ...
                // ✅ Feature 3: path polyline cleanup
                if (busPolylines[groupId]) {
...
// Fixed blue route polyline — OSRM ছাড়া, নিজের coordinates দিয়ে
function updateEtaAndRoute(groupId) {
    if (routePolylines[groupId]) return; // আগে থেকেই আছে
    const path = routePaths[groupId];
    if (!path) return; // এই route এর path নেই
...
if (mySessionId) {
    try {
        ...
        if(routeInfo && routeInfo.from && routeInfo.to) {
            ...
            // 🔒 Privacy Fix: confirm করে resume
